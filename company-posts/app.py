...
def get_company_posts(event, context):
    ...
    PK = _get_posts_meta_keys(company_id)
    print("Key: ", json.dumps({'PK':PK}, indent=4))
    
    
    try:
        response = table.query(
                KeyConditionExpression=Key('PK').eq(PK),# & Key('shared_on').eq(shared_on),
...
def _get_posts_meta_keys(company_id):
